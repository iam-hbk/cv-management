...
export default function Dashboard() {
	...
	const recentJobSeekers = jobSeekers?.filter((js) => js._creationTime > sevenDaysAgo).length ?? 0;
	const recentVacancies = vacancies?.filter((v) => v.createdAt > sevenDaysAgo).length ?? 0;

	return (
		<div className="space-y-6">
			<div>
				<h1 className="text-3xl font-bold">Dashboard</h1>
				<p className="text-muted-foreground">Welcome to the IntoBeing Placements Admin Panel</p>
			</div>
			...
			<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
				<Card>
					<CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
						<CardTitle className="text-sm font-medium">Pending Vacancies</CardTitle>
					...
					<CardContent>
						{isLoading ? (
							...
							<div className="text-2xl font-bold text-yellow-600">{pendingVacancies.length}</div>
						)}
				...
				<Card>
					...
					<CardContent>
						{isLoading ? (
							...
							<div className="text-2xl font-bold">{recentJobSeekers + recentVacancies}</div>
						)}
				...
				<Card>
					...
					<CardContent>
						{isLoading ? (
							...
							<p className="text-muted-foreground text-center py-4">No pending vacancies</p>
						) : (
							<div className="space-y-3">
				...
				<Card>
					...
					<CardContent>
						{isLoading ? (
							<div className="space-y-3">
								{[...Array(3)].map((_, i) => (
									<Skeleton key={`skeleton-seeker-${i}`} className="h-12 w-full" />
								))}
							</div>
						) : jobSeekers.length === 0 ? (
							<p className="text-muted-foreground text-center py-4">No job seekers yet</p>
						) : (
							<div className="space-y-3">
