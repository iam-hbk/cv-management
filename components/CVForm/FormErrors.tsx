...
export default function FormErrors({ errors }: { errors: FieldErrors }) {
	const renderErrors = (error: ErrorWithMessage, path: string = ""): ReactElement[] => {
		...
		if (error.message && typeof error.message === "string") {
			elements.push(
				<Alert variant="destructive" key={path}>
					...
					<AlertDescription>{path ? `${path}: ${error.message}` : error.message}</AlertDescription>
				</Alert>
			);
		}
		...
		Object.entries(error).forEach(([key, value]) => {
			if (typeof value === "object" && value !== null && key !== "ref" && key !== "types") {
	...
	return <div className="space-y-2">{renderErrors(errors)}</div>;
}
