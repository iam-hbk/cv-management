...
export function JobSeekersCVTable() {
	...
	const filtered = useMemo(() => {
		...
		if (search.trim()) {
			...
			list = list.filter(
				(r) =>
					`${r.jobSeeker.firstName} ${r.jobSeeker.lastName}`.toLowerCase().includes(q) ||
					r.jobSeeker.email.toLowerCase().includes(q)
			);
		}
		if (statusFilter !== "all") {
		...
		() => [
			{
				id: "name",
				header: "Name",
				accessor: (r) => `${r.jobSeeker.firstName} ${r.jobSeeker.lastName}`.trim() || "-",
			},
			{
				id: "email",
				header: "Email",
